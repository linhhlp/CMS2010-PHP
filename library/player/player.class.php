...
class player {
	...
	//Include setting:
		...
		$player = " <script type='text/javascript' src='".$mod_path."/swfobject.js'></script>	
			<div id='".$div."'>Rất tiếc bạn hok nghe được nhạc nền, hãy thử Refresh  trang web</div>
			  <script type='text/javascript'>
			  var s1 = new SWFObject('".$mod_path."/player.swf','ply','".$width."','".$height."','9','#');				// width 	height
	...
	static function replacePlayer ($string,$compare='') {
		while(is_int ( stripos($string,'[VIDEO]') ) || is_int ( stripos($string,'[AUDIO]')) || is_int ( stripos($string,'[PLAYLIST]') )  ) {
			...
			elseif(is_int ( stripos($string,'[PLAYLIST]') )) {
				...
				$compare = substr($string,stripos($string,'[AUDIO]') ,stripos($string,'[/AUDIO]') - stripos($string,'[AUDIO]') + 8);
			}
