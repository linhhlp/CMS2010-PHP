...
if($community_setting["ENABLE"] == TRUE) {
	...
	foreach($friend_list as $key=>$value) {
		...
		if($url_sender == $friend_list_split[2]) {
			...
			$temp_array = explode(":-:",$_POST['belong']);//pre($temp_array);
			$get_id 	= user::getIDByUser($temp_array[0]);
			$userbonus	= new userbonus(); 
			$suc = $userbonus->createBounus($_POST['title'],$_POST['content'],$_POST['created'],$get_id,$_POST['public'],$value['id'].":".$created_by	);
			if( $suc != false ) {
				$complete = TRUE;
				//pre($_POST);
