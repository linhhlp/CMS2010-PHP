...
if( $send_param[0] > 0) {
	...
	$parse_url = parse_url($friend[3]);
	$httpclient = new httpclient($parse_url['host']);//echo $parse_url['path'].'xml.php';die;
	//$httpclient->setDebug(1);
	$httpclient->post($parse_url['path'].'xml.php?type=community&name=makefriend',$data);
	//echo $httpclient ->getContent();
	$result = xmlParse($httpclient ->getContent());
	// 
	//pre($result);die;
	...
	if($_POST['url'] != "") {
		...
		if( $id_insert != FALSE ) {
			...
			$parse_url = parse_url($send_to);//pre($parse_url);
			...
			if($result['completed'] != "FALSE" ) {
				...
				echo "Không thể gửi lời mời, vui lòng kiểm tra lại kết nối cũng như địa chỉ trang web";
			}
		}
		else {
		...
		echo "<tr><td width=30%> </td><td><input name='friendname' value='".$user_info['user_name']."' type='hidden' /> </td></tr>";
