...
elseif($type == 'edit')			// Edit
{
	...
	$arr = array( 'type'=>'Front page', 'limit_start'=>$limit_Frontpage_start, 'limit'=> $config [1] ,'order'=>'last_change' );
	$comment_arr = $comment->getComments ( $arr );
	$num_com  = $comment_arr ['total'];//$comment->getNumCom();
	$blog_comments = $comment_arr['parent'];
	 	...
	 	foreach($blog_comments as $key=>$value)
		{
			...
			if( !empty($comments_child[ $value['id']])) {
				foreach ( $comments_child[ $value['id']] as $key2=>$value2) {
					if(empty($user_name[$value2['user']])) $own_comment =  $value2['user'];
					else $own_comment =  $user_name[$value2['user']];
					if($value2['publish'] == 1) $publish = 'Everybody';
						elseif($value2['publish'] == 2) $publish = 'Friend';
						else $publish = 'Private';
					echo	"<tr><td>&nbsp;</td>";
					echo "<td>".$value2['title']."</td><td>".$own_comment."</td>";
					echo "<td><font color=yellow>".$value2['comment']."</font></td>";
					echo "</td>
						<td>". $publish."</td>
						<td><a href='".urlmanager::makeLink($PARAS,'edit-'.$value2['id'],$CONTROL)."'><img border=0 align=right valign=bottom src='"._URL.IMA."/edit.png' title='Sửa bài viết này'></a></td>
						<td><a href='".urlmanager::makeLink($PARAS,'delete-'.$value2['id'],$CONTROL)."'><img border=0 align=right valign=bottom src='"._URL.IMA."/del.png' title='Xóa bài viết này'></a></td>
						</tr>";
				}
			}
		}
