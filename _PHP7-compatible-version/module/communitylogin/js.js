...
function xacthuc( captcha ) {
	if( captcha != "" ) {
		var ans = prompt(captcha,"");
		if( ans != null ) {
			var ajax = new ajax_connect();
			var addr = address + '?type=xacthuc&ans='+ans+'&q=1102';
			ajax.open("GET",addr);
			ajax.onreadystatechange = function() {
				if(ajax.readyState == 4 && ajax.status == 200) {
					Result = ajax.responseText; 
					var xmldoc	 	= processStrXML(Result);//alert(news_info);
					var message 	= xmldoc.getElementsByTagName("error")[0].childNodes[0].nodeValue;
					if( message == 1 ) {
						alert("Sai rồi, câu trả lời là chữ tiếng Việt không dấu");
						//xacthuc( captcha );
						location.reload();
					}
					else {
			...
			//alert("Bạn không thể dùng 1 số chức năng như bình luận.");
...
function ajax_process(address,GETorPOST,str){
    ...
    if(GETorPOST == "GET") {
        ...
        ajax.open("POST",address,true);//alert(address);
...
form_login_str		= form_login_str + '<label for="name">Tên của bạn: </label><input name="user" value="" type="text" autofocus/><br /><input type="checkbox" name="remember_me" value="1" checked=1>Đăng nhập dài hạn  ';
...
form_login_str		= form_login_str + '<input name="logintype" value="firststep" type="hidden"/><input name="privatekey" value="'+privatekey+'" type="hidden"/><input name="prefix" value="'+prefix+'" type="hidden"/>';
form_login_str 		= form_login_str + '<label for="submit"><input type="submit"  value="Tham gia" name="submit" ';
...
form_member_login_str = form_member_login_str +  'Nhập password (đăng nhập như thành viên) <br />Nhấn "hủy" để chọn tên đăng nhập mới';
...
form_member_login_str = form_member_login_str + '<input name="pass" value="" type="password" autofocus/>';
...
form_member_login_str = form_member_login_str + '<input type="submit"  value="Đăng nhập" name="submit" onclick="ajax_process('+ "'" +address + " ','POST', getformvalue('form_userbonus')" + ')"/> ';	//Đăng nhập
form_member_login_str = form_member_login_str + '</form> ';
    // Hien Cancel
form_member_login_str = form_member_login_str + '<input type="button" onclick="hien_thi_div(\'mess_userbonus\',form_login_str);" value="Hủy"> ';
    ...
    ajax.onreadystatechange = function() {
        if(ajax.readyState == 4 && ajax.status == 200) {
        	...
        	if(post == 'true') {
        		...
        		strout = '<div class="COMMENT_HLP_A_ITEM">Tiêu đề:<span class="COMMENT_HLP_TITLE"> '+title+'</span> <span class="COMMENT_HLP_USER">(của tôi vừa gửi)</span></div><div class="COMMENT_HLP_COMMENT">'+content+'</div>';
        	}
        	else strout = 'Không gửi được bình luận của bạn! Kiểm tra lại kết nối!';
