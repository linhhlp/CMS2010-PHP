...
class user {
	...
	static public function getUserName($id='',$gettypr='user',$input='id') {
		$db		  = new DBmanager();
		
		$arr = array('"',"'"," ",";");	// tự d?ng b? ' v� " v� 1 s? k� t? d?c bi?t ra kh?i string
		...
		if( $id != '' ) {
			$id	= '"'.str_replace(',','","',$id).'"';
			if($input == 'id') $user_que .= ' WHERE `id` IN ('.$id.') ';
			else if($input == 'user') $user_que .= ' WHERE `user` IN ('.$id.') ';
		}
		$user_res = $db->execute($user_que);//echo $user_que; echo mysqli_error();
		$user_num = $db->count();
		if($user_num > 0) {
	...
	// Dạng users là
	// hlp,thomay,abcuser,...
	// chú ý, ko có dấy ' để ngăn cách string
	static public function getUsers($users) {
		$db	= new DBmanager();
		$arr = array('"',"'"," ",";");	// tự động bỏ ' và " và 1 số kí tự đặc biệt ra khỏi string
		$users	= str_replace($arr,'',$users);
		$users	= trim($users,',');
		$users	= $db->escapeString($users);
		if( $users == '' ) return FALSE;
		$users	= '"'.str_replace(',','","',$users).'"';
		$user_que = 'SELECT * FROM '.PREFIX.'user WHERE `user` IN ('.$users.') ';
		$user_res = $db->execute($user_que);
		//echo $user_que;
		$n = $db->count();//echo $n;
		if($n >= 1) {
