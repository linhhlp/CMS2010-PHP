...
class visitor {
	...
	public function Visit() {
		...
		if(!isset($_SESSION['status_ip']))		// This is new visit
		{
			...
			$Them_IP = "INSERT INTO   ".PREFIX."visit   (`time` , `ip`,`name`,`info`) VALUES ('".$time."' , '$this->ip','".$user_info['user_name']."','$last_url')";
			$db->execute($Them_IP);
			...
			$db->execute("UPDATE   ".PREFIX."visit   SET   `name`='".$db->escapeString($user_info['user_name'])."' , `info`='$info'  WHERE `id`='".$db->escapeString($_SESSION['status_ip'])."'");
		}
		return $this->visit;
	}
