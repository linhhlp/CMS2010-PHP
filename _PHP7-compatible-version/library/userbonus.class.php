...
class userbonus {
	...
	public function getBonus($id ='',$belong = '' , $start=0 , $limit=3, $public= 1,$created_by='') {
		...
		if($num > 0) {
			...
			for($i=0; $i<$num; $i++) {
				...
				if($user_info['id'] == $row['belong'] || $user_info['id'] == $row['created_by'] ) {
	...
	public function createBounus($title,$content,$created,$belong,$public=1,$created_by="") {
		//pre(func_get_args());
		if ( !isset($login)) global $login;
		if ( isset($login)) $user_info = $login->getInfo(); 
		...
		if($belong >0 ) {
			...
			$belong 	= $db->escapeString($belong);
			$public 	= $db->escapeString($public);
			if($created_by == '') $created_by = $user_info['id'];
			//echo $fulltext;
	...
	public function sendBounus($title,$content,$created,$belong,$public=1,$created_by="") {
		...
		$parse_url = parse_url($url_friend);//pre($parse_url);
		...
		$httpclient->post($parse_url['path'].'xml.php?type=community&name=userbonus',$data);
	...
	public function deleteBonus($id) {
		...
		if($user_info['type'] != '3' && $user_info['type'] != '4' ) {
			...
			if($info[0]['belong'] != $user_info['id']) return FALSE;
		}
