...
echo	"Quản lí tin nhắn của bạn<br><br />";

echo urlmanager::makeLink($url[0],'inbox','','inbox','Hòm thư');
echo '<br /><br />';
echo urlmanager::makeLink($url[0],'sent','','sent','Tin đã gửi.');
...
if($url[1] == 'new') {
	...
	// This is normal display : full capability browser (in windows or Linux ...)
		...
		echo "
			...
			<tr><td>Nội dung*</td><td><textarea rows=15 name=content cols=28 id=fulltext></textarea></td></tr>
			<tr><td>Người nhận*</td><td>";
		// get member
		...
		if($community_setting["ENABLE"] == TRUE) {
			...
			foreach($friend_list as $key=>$value) {
				...
				echo "<option value='".$value['value2'].":-:".$friend_list_split[2]."' ".($rely==$value['id']?'selected=selected':'').">".$friendtype_2[1].": ".$value['value2']." ở ".$friend_list_split[2]."</option>";
			}
		}
		
	 	echo "</select>";
		echo "</td></tr>";
		echo "<tr><td>Ngày gửi*</td><td><input name=created type=text value='$date_viet'></td></tr>
			<tr><td>Gửi đi</td><td><input value=submit type=submit></td></tr>
			</table>
			</form>
		";
	}
...
elseif ($url[1] == 'read') {
		...
		echo "
			...
			<tr><td>Tiêu đề</td><td>".$content['title']."</td></tr>
			<tr><td colspan=2>Nội dung:</td></tr>
			<tr><td colspan=2><div class=userbounus_content>".$content['content']."</div></td></tr>
			<tr><td>Từ </td><td>";
		$all_user = user::getUserName('','id');
		$created_by = explode(':',$content['created_by']);
		if( count($created_by) == 1 ) $name_user_belong = $all_user[$content['created_by']];
		else {
			$get_config = $community->getConfig('','',$created_by[0]);
			$get_config = $get_config[0];
			$get_config_arr = explode(':-:',$get_config['value3']);
			$get_config_arr_0 = explode(':_-_:',$get_config_arr[0]);
			$name_user_belong = $get_config_arr_0 [1].' '.$get_config['value2'].' - '.$get_config_arr[2];
		}
		
		echo $name_user_belong;//$all_user[$content['created_by']];
		echo "</td></tr>
			<tr><td>Ngày nhận</td><td>".time2TiengViet($content['created'])."</td></tr>
...
elseif ($url[1] == 'inbox') {
	...
	$content = $userbonus->getBonus('',$user_info['id'],$limit_start,$config [1],'','');
	...
	if($content != FALSE) {
		echo "<table border=1 width=95% style='border:2px dashed #008000; border-collapse:collapse' bordercolorlight='#00FF00' bordercolordark='#008000'>
				<tr><td width=5%>#</td><td width=5%>Tiêu đề</td><td>Nội dung</td><td  width=10%>Từ</td><td  width=5%>Trạng thái</td><td  width=5%>Xóa</td></tr>";
		$i = 1;
		$all_user = user::getUserName('','id');
		foreach ($content as $key=>$value) {
			...
			$created_by = explode(':',$value['created_by']);
			if( count($created_by) == 1 ) $name_user_belong = $all_user[$value['created_by']];
			else {
				$get_config = $community->getConfig('','',$created_by[0]);
				$get_config = $get_config[0];
				$get_config_arr = explode(':-:',$get_config['value3']);
...
elseif ($url[1] == 'sent') {
	...
	$content = $userbonus->getBonus('','',$limit_start,$config [1],1,$user_info['id']);
	$num_com  = $userbonus->getNumMessage();
	if($content != FALSE) {
		echo "<table border=1 width=95% style='border:2px dashed #008000; border-collapse:collapse' bordercolorlight='#00FF00' bordercolordark='#008000'>
				<tr><td width=5%>#</td><td width=5%>Tiêu đề</td><td>Content</td><td  width=10%>Tới</td><td  width=5%>Trạng thái</td></tr>";
		$i = 1;
		$all_user = user::getUserName('','id');
		foreach ($content as $key=>$value) {
			$extract = wrap_content($value['content'],0,$config [0]);
		...
		echo 'Bạn chưa có tin đã gửi nào!';
	}
}
else {
	$title_meta_data_temp = "Home";
	$new = $userbonus->isNew($user_info['id']);
	if( $new != false ) echo "Bạn có ".count($new)." tin nhắn mới. <br/>";
