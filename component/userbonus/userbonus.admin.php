...
if($_POST['userbonus_params_num'] != '') {
	...
	$check_register_query = "SELECT * FROM `".PREFIX."community` WHERE  `type`='community' and `name`='userbonus'";
	$db->execute($check_register_query);
	if($db->count() == 0 ) {
		echo " register";
		$db->execute("INSERT INTO `".PREFIX."community` (`name`,`type`) VALUES ('userbonus','community')");
	}
