...
echo "<tr><td>Trả lời lời mời</td><td><form action='".urlmanager::makeLink($url[0], "send", $url[2])."' method='post'>
	Nickname của bạn: <input type='text' name='my_name' value='".$user_info['user_name']."'/><br />
	Đồng ý hay không: <select name='reply'><option value='deny'>deny</option><option value='accept' selected >accept</option></select><br />
	Xác nhận :<input value='Gửi' type='submit' /> <input value='Xóa' type='reset' /></form>
 </td></tr>";
}
