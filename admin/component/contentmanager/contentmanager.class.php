...
class contentmanager {
	...
	public function getContent($id = '',$limit_from = 0,$limit_to= 1,$category ='',$publish = '',$created='',$created_by='', 	$modified =''	,$modified_by =''	,$hit 	='',$frontpage = '') {
		...
		if($publish == '')	{
			...
			if($_SESSION['type'] == "4"||$_SESSION['type'] == "3") {
				$publish = '';
			}
	...
	/*
	public function getContent($id='',$pages = 0,$perpage=20) {
		$db 		= new DBmanager();
		$perpage	= $db->escapeString($perpage);
		$pages 		= $db->escapeString($pages);
		if($pages > 1) $pages = $pages - 1;
		else $pages = 0;
		$page_start = $pages * $perpage;
		$id 		= $db->escapeString($id);
		$query		= 'SELECT * FROM '.PREFIX.'blog ';
		if($id != '') $query .= 'WHERE `id`="'.$id.'"';
		$query	   .= ' ORDER BY created DESC ';
		$query     .= 'LIMIT '.$page_start.','.$perpage. ' ';
		$db->execute($query);
		$num  		= $db->count();
		if($num > 0) {
			$results = array();
